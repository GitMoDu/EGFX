const BitmaskFontDeclaration = (() => {
  ...
  function generate(state, characterForGlyphId, packGlyph, sourceNamespace, fontName) {
	let offset = 0;
	const cases = state.glyphs.flatMap(glyph => {
	  const character = characterForGlyphId(glyph.id || '');
	  const glyphOffset = offset;
	  offset += packGlyph(glyph, state.height).length;
	...
	return [
	  ...
	  '\tdimension_t GetFontWidth() const { return ' + sourceNamespace + '::MaxWidth; }',
	  ...
	  '\tdimension_t GetTabWidth() const { return ' + sourceNamespace + '::MaxWidth * 2; }',
	  '\tdimension_t GetKerningWidth() const { return ' + sourceNamespace + '::MaxWidth / 3; }',
