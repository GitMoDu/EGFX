#ifndef _INTEGERGLASS_FRAMEWORK_LAYOUT_CONSTRAINED_RATIO_h
#define _INTEGERGLASS_FRAMEWORK_LAYOUT_CONSTRAINED_RATIO_h
...
namespace IntegerGlass
{
	namespace Framework
	{
		namespace Layout
		{
			...
			private:
				static constexpr int16_t AvailableWidth() { return ParentLayout::Width(); }
				static constexpr int16_t AvailableHeight() { return ParentLayout::Height(); }
			...
			public:
				...
				static constexpr int16_t Width()
				{
					return HeightLimited()
						? static_cast<int16_t>((uint32_t(AvailableHeight()) * uint32_t(AspectWidth)) / uint32_t(AspectHeight))
						: AvailableWidth();
				}
				...
				static constexpr int16_t Height()
				{
					return HeightLimited()
						? AvailableHeight()
						: static_cast<int16_t>((uint32_t(AvailableWidth()) * uint32_t(AspectHeight)) / uint32_t(AspectWidth));
				}
