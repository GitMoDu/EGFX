...
class ScreenST7789_X_X
	: public virtual IFrameSink
	, public virtual IFramePrimitives
{
protected:
	DisplayST7789_135x240x16_SPI DriverSpi;

public:
	ScreenST7789_X_X()
		: IFrameSink()
		, IFramePrimitives()
		, DriverSpi(pinRST, { spiChannel, {pinCS}, pinDC, ST7789SpiSpeed, -1, -1 })
	{
	}
...
public: // IFrameSink overloads.
	...
	virtual const bool Start() final
	{
		...
		DriverSpi.getInterface().setOffset(0, 0);
		DriverSpi.clear();
...
public: // IFrameBufferPrimitives overloads.
	...
	{
		//DriverSpi.clear();
		DriverSpi.setColor(0);
		DriverSpi.fillRect(0, 0, screenWidth - 1, screenHeight - 1);
	}
	...
	virtual void Line(const uint8_t x1, const uint8_t y1, const uint8_t x2, const uint8_t y2, const RgbColor& color) final
	{
		...
		else if (y1 == y2)
		{
			...
			//DriverSpi.drawLine(x1, y1, x2, y1);
		}
		else
		{
			DriverSpi.setColor(RGB_COLOR16(color.r, color.g, color.b));
	...
	virtual void Rectangle(const uint8_t x1, const uint8_t y1, const uint8_t x2, const uint8_t y2, const RgbColor& color, const uint8_t width) final
	{
		...
		for (uint_fast8_t i = 0; i < width; i++)
		{
			...
			DriverSpi.setColor(RGB_COLOR16(color.r, color.g, color.b));
			DriverSpi.drawLine(x1, y1, x2 - 1, y1);
			DriverSpi.drawVLine(x1, y1 + 1, y2 - 2);
			DriverSpi.drawVLine(x2 - 1, y1 + 1, y2 - 2);
			DriverSpi.drawLine(x1, y2 - 1, x2 - 1, y2 - 1);
		}
	}
...
template<const uint8_t spiChannel,
	...
	const uint8_t pinDC,
	const uint8_t pinRST>
class ScreenST7789_240_240 : public ScreenST7789_X_X<spiChannel, pinCS, pinDC, pinRST, 240, 240>
{
public:
	ScreenST7789_240_240()
		: ScreenST7789_X_X<spiChannel, pinCS, pinDC, pinRST, 240, 240>()
	{}
};
#endif