...
namespace Egfx
{
	struct SpriteRenderer
	{
		...
		{
			if ((positionY + line < 0)
				|| ((positionY + line) >= frame->GetFrameHeight()))
			{
				return;
			}
			...
			const pixel_t column)
		{
			if ((positionX + column < 0)
				|| ((positionX + column) >= frame->GetFrameWidth()))
			{
				return;
			}
			...
			const pixel_t positionX, const pixel_t positionY)
		{
			...
			const pixel_t xEnd = MinValue(frame->GetFrameWidth(), (pixel_t)(positionX + sprite->GetWidth()));
			const pixel_t yEnd = MinValue(frame->GetFrameHeight(), (pixel_t)(positionY + sprite->GetHeight()));
