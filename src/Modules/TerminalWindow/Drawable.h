		...
		namespace TerminalWindow
		{
			namespace Drawable
			{
				...
				public:
					...
					void Draw(IFrameBuffer* frame, const rgb_color_t color, const pixel_t offsetX)
					{
				...
				{
				private:
					using CursorDrawableType = Drawable::Cursor<ParentLayout, FontWidth, FontHeight>;
					CursorDrawableType CursorDrawable{};

				public:
					static constexpr auto CharactersPerLine = ParentLayout::Width() / (FontWidth + Kerning);
					static constexpr auto Lines = ParentLayout::Height() / (FontHeight + LineSpacing);
				...
				public:
					Framework::Text::TemplateWriter<ParentLayout, FontDrawerType> TextWriter{};
				...
				public:
					...
					void Clear()
					{
						...
						CursorX = 0;
						OffsetY = 0;
						LastNewLineTime = micros();
						LastCursorTime = LastNewLineTime;
					}
				...
				public: // Drawable interface
					void Draw(IFrameBuffer* frame, const uint8_t line)
					{
						...
						TextWriter.Write(frame,
							ParentLayout::X(),
							ParentLayout::Y() + ParentLayout::Height() - lineOffsetY + OffsetY,
							reinterpret_cast<const char*>(&TextBuffer[Lines - line]));

						if (line >= Lines - 1
							&&
							((micros() - LastCursorTime) % Durations::CursorBlinkPeriod <= (Durations::CursorBlinkPeriod / 2))
							)
						{
							// Draw cursor on last line.
							CursorDrawable.Draw(frame, RGB_COLOR_WHITE,
								ParentLayout::X() + (CursorX * (FontWidth + Kerning)));
						}
					}
				...
				private:
					...
					void Push(const uint8_t character)
					{
						...
						LastCursorTime = micros(); // Update cursor time to prevent blinking during typing.
					}
					...
					void NewLine()
					{
						...
						LastCursorTime = LastNewLineTime; // Update cursor time to prevent blinking during animation.
					}
				};
			}
		}
